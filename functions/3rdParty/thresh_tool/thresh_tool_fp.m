function [level,bw] = thresh_tool_fp(im,cmap,defaultLevel) %mainfunction
% thresh_tool_pan_zoom_fp.m: Tool for interactive selection of an image
% threshold value, applied in this case to selection of an estimated floodplain 
% elevation. This function is based on thresh_tool.m 1.6.0.0
% by Robert Bemis, from the MATLAB file Exchange
% (https://www.mathworks.com/matlabcentral/fileexchange/6770-thresholding-tool).
% The original documentation follows.

% ind=pixel_map==-9999;
% pixel_map(ind)=min(pixel_map(~ind)); % set any unassigned to lowest value of pixel map so non-convergent pixels don't set a threshold
...
fh=image(im); axis image
load hillshade.mat H
set(fh,'AlphaData',H)
...
%segmented image
bw = or(isnan(im),im<my_level);
axes(h_ax2)
hold on
fh2=subimage(bw); axis off, axis ij
set(fh2,'AlphaData',H)
cd = get(fh,'CData');
% change to blue overlay by clearing red, green
cd(:,:,1)=0;
cd(:,:,2)=0;
set(fh2,'Cdata',cd);
clear cd
