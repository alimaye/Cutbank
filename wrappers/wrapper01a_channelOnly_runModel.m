% wrapper01a_channelOnly_runModel.m: Wrapper script for demonstrating
% model execution for a case with only channel evolution (i.e., no bank-
% material tracking).
...
t_max = 1000; % Run time (yr)
t_increment = 2.5; % Time increment (yr)
...
BMT = 'channel-only';  % 'channel-only' | 'vector-based' | 'grid-based'. Method for bank-material tracking (BMT)
chkpt_spacing_coeff = false; % Vector-based BMT only: controls the spacing of checkpoints along a bank-material inspection vector.
bed_elev_chg_poly_addn_thresh = NaN; % Vector-based BMT only: cumulative vertical erosion before a new polygon for tracking bank materials is created (m)

% 2. Channel geometry and kinematics
w = 20; % Channel width, m.
D = 1; % Channel depth, m
init_centerline_type = 'straight'; % 'straight' | 'evolved' | 'sinusoidal' | 'custom'
init_centerline_file = ''; % Name of file that stores initial coordinates of channel centerline (optional).
...
k_erode_sediment = 1; % Sediment erosion rate coeffient. Yields this maximum erosion rate in m/yr.
...
bed_elev_chg_rate = 0; % Bed elevation change rate, m/yr (negative for erosion, positive for aggradation).
vertical_file = ''; % File for importing a predefined vertical incision rate time-series  (optional).
enforceChannelDisplacementLimit = false; % Logical switch that sets whether or not to throw an error if channel dispacement in one time step exceeds one channel width. 'true' | 'false'.

% 3. Meandering model constants
...
% 6. Output files
trial = 'test_01'; % Base name for file output
save_interval = 100; % File output interval (yr)

% % The remaining variables only apply for model runs with vector-based bank
% % material tracking enabled.

% Switch for modifying lateral erosion rates due to bedrock.
bedrock_erosion.modify_lateral_erosion_bedrock = false; 
k_erode_bedrock = NaN; % Bedrock erosion rate coeffient. Yields this erosion rate in m/yr.
unconfined_alluvial_belt_width = NaN; % The width of the alluvial belt under unconfined conditions.
init_alluv_width_coeff = NaN; % The imposed alluvial belt width, as a fraction of the previous variable.
...
overbank_deposition.modify_lateral_erosion_resistant_oxbows = false;
overbank_deposition.modify_lateral_erosion_bank_height = false;

% Parameters for Howard (1992) overbank deposition model.
overbank_deposition.parameters.nu = NaN; % Rate of spatially uniform overbank deposition (m/yr)
overbank_deposition.parameters.mu_d = NaN; % Maximum rate of space-dependent overbank deposition (m/yr)
overbank_deposition.parameters.lambda = NaN; % e-folding length for exponential decay in space-dependent overbank deposition rate.
...
%%% Set file paths to rest of code, and create output directory
