% wrapper03a_bankMaterialsGridded_runModel.m: Wrapper script for
% demonstrating model execution for a channel with grid-based bank-
% material tracking.
...
domain.xExtentChannelWidths = 200; % Extent of model domain, in channel widths.
centerline_spacing_coeff = 1; % Coefficient that sets node spacing coefficient along channel centerline, in channel widths.
BMT = 'grid-based'; % 'channel-only' | 'vector-based' | 'grid-based'. Method for bank-material tracking (BMT)
cell_width = 10; % Grid-based BMT only: width of grid cells in m.


% 2. Channel geometry and kinematics
w = 20; % Channel width, m.
D = 1; % Channel depth, m
init_centerline_type = 'sinusoidal'; % 'straight' | 'evolved' | 'sinusoidal' | 'custom'
...
bedrock_erosion.modify_lateral_erosion_bedrock = false; % Switch for modifying lateral erosion rates due to bedrock.
k_erode_bedrock = 1; % Bedrock erosion rate coeffient. Yields this erosion rate in m/yr.
unconfined_alluvial_belt_width = 50*w; % The width of the alluvial belt under unconfined conditions.
init_alluv_width_coeff = 0; % The imposed alluvial belt width, as a fraction of the previous variable.
...
overbank_deposition.parameters.lambda = w; % e-folding length for exponential decay in space-dependent overbank deposition rate.
...
% 7. Output files
trial = 'test_03'; % Base name for file output
save_interval = 100; % File output interval (yr)

% These parmaeters are specific to 3-dimensional stratigraphic modeling,
% which is not enabled here.
stratigraphy_3D.enable = false;
stratigraphy_3D.delta_z = NaN; % vertical spacing of voxels
stratigraphy_3D.zMin = NaN;
stratigraphy_3D.zMax = NaN;  %%% can add an option for cumulative sum....also will change from vertical incision to bed change rate
% Uses low-memory data type (uint8) to track stratigraphic units. 0 = air, 1 = sand, 2 = mud.
...
switch BMT
    case {'channel only','grid-based'}
        chkpt_spacing_coeff=NaN;
        bed_elev_chg_poly_addn_thresh=NaN;
     case {'vector-based'}
end

if stratigraphy_3D.enable
    estimated_strat_cells = ((x_extent_widths*w/cell_width)^2)*(range([stratigraphy_3D.zMin stratigraphy_3D.zMax])/stratigraphy_3D.delta_z);
    bytes_per_strat_cell = 1; % for uint8 data type
    estimated_strat_memory_MB = estimated_strat_cells*bytes_per_strat_cell / 1e6; % approximate (conventions differ slightly for definition of megabyte)

    % Ask user if the requested memory footprint is acceptable
    str = input(sprintf('The memory estimate for the stratigraphy grid is %4.2f MB. Proceed? [y/n] ',estimated_strat_memory_MB),'s');
    if ~strcmp(str,'y')
        fprintf('Aborted model run\n')
        return
    end
end
    
% Only one of the processes that can modifies lateral erosion rates (i.e., bedrock,
% resistant oxbows, or bank height set up by overbank deposition) should
% be enabled at one time. Here, check that only one of these
